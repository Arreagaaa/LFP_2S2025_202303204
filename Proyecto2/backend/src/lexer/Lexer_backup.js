...
const Character = require("./utils/Characters.js");
...
class Lexer {
  ...
  #S1() {
    ...
    while (
      Character.isAlphanumeric((this.#next_char = this.#input[this.#pos_char]))
    ) {
  ...
  #S5() {
    ...
    if (!Character.isDigit((this.#next_char = this.#input[this.#pos_char]))) {
      this.#addError(
        "Se esperaba al menos un dígito después del punto decimal"
      );
      return null;
    }
  ...
  #S23() {
    ...
    this.#addError(
      'Operador "!" no válido en Java. Use "!=" para desigualdad.'
    );
    return null;
  }
